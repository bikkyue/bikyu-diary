...
bikyu-diaryでは、Cloudflare R2を使用した画像の保存と表示機能を提供しています。
...
```json
{
  "url": "https://r2diary.bikyu.dev/images/2026021000250001.jpg",
  "key": "images/2026021000250001.jpg"
}
```
...
### アップロード後のURL（絶対URL）
- 形式: `https://r2diary.bikyu.dev/images/yyyymmddHHMMSS01.{拡張子}`
- 例: `https://r2diary.bikyu.dev/images/2026021000250001.jpg`
...
- [x] 画像削除API（`DELETE /api/images/{key}`）
- [x] マークダウン内画像の表示
...
- `src/pages/[...slug].astro` - 日記詳細（本文内画像表示）

### 設定
- `wrangler.toml` - R2バケット設定

### ドキュメント
- `document/画面設計書/admin-images.md` - 画像管理画面仕様
- `document/画面設計書/diary-detail.md` - 詳細画面仕様
